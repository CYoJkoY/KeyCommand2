...
public class KeyCommandMod {
    ...
    public static class GlobalEventListener {
    	...
    	// 自动追怪处理
        private final GuiInventory.AutoFollowHandler autoFollowHandler = new GuiInventory.AutoFollowHandler();
    	
        @SubscribeEvent
        public void onPlayerTick(TickEvent.PlayerTickEvent event) { // 全局玩家Tick
    ...
    public static class GuiInventory extends GuiScreen {
        ...
        // 新增自动进食相关字段
        private static final File EAT_CONFIG_FILE = new File("config/KeyCommand/keycommand_autoeat.json");
        ...
        private static double customGuiScale = 4.0;
    	...
    	// 自动追怪配置
        private static final File FOLLOW_CONFIG_FILE = new File("config/KeyCommand/keycommand_autofollow.json");
        ...
        private static double minX = Math.min(point1.x, point2.x);
        private static double maxX = Math.max(point1.x, point2.x);
        private static double minZ = Math.min(point1.z, point2.z);
        private static double maxZ = Math.max(point1.z, point2.z);
        private static boolean isMovingToPoint = false;
        ...
        private static void loadSkillConfig() {
            try {
                if (SKILL_CONFIG_FILE.exists()) {
                    ...
                    // 读取自动技能总开关状态
                    if (json.has("autoSkillEnabled")) {
                        autoSkillEnabled = json.get("autoSkillEnabled").getAsBoolean();
                    } else {
                        autoSkillEnabled = false; // 默认值
                    }
        ...
        public static void saveSkillConfig() {
            try {
                ...
                json.addProperty("autoSkillEnabled", autoSkillEnabled);
        ...
        private static void loadAutoEatConfig() {
            try {
                if (EAT_CONFIG_FILE.exists()) {
                    JsonObject json = new JsonParser().parse(new FileReader(EAT_CONFIG_FILE)).getAsJsonObject();
        ...
        public static void saveAutoEatConfig() {
            try (FileWriter writer = new FileWriter(EAT_CONFIG_FILE)) {
                JsonObject json = new JsonObject();
        ...
        public GuiInventory() {
            ...
            SetItems.add("followconfig"); SetItemNames.add("自动追怪");
        ...
        public static void initializePathSequences() {
            ...
            Daily1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(980, 776, true));
            Daily1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(993, 775, true));
            ...
            At1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(1160, 400, true)); // 10min
            At1.addAction(new DelayAction(6));
            At1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(1240, 400, true)); // 10min
            
            At1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(1200, 480, true)); // 30min
            At1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(1200, 570, true)); // 60min
            At1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(1200, 650, true)); // 120min
            At1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(1200, 730, true)); // 240min
            At1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(1200, 820, true)); // 480min
            At1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(1200, 900, true)); // 600min
            ...
            GetEmail1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(533, 355, true)); // 第一个邮件
            GetEmail1.addAction(new DelayAction(6));
            GetEmail1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(750, 355, true)); // 第一个邮件
            GetEmail1.addAction(new DelayAction(6));
            GetEmail1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(1340, 1000, true)); // 领取附件
            GetEmail1.addAction(new DelayAction(6));
            GetEmail1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(960, 618, true)); // 领取并删除
            ...
            DelEmail1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(533, 355, true)); // 第一个邮件
            DelEmail1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(750, 355, true)); // 第一个邮件
            DelEmail1.addAction(new DelayAction(6));
            DelEmail1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(805, 405, true)); // 删除
            DelEmail1.addAction(new DelayAction(6));
            DelEmail1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(965, 625, true)); // 确认删除
            ...
            Refine1.addAction(player -> GuiInventory.simulateMouseClickByBottom(1100, 830, true)); // 精炼物品
            Refine1.addAction(new DelayAction(6));
            Refine1.addAction(player -> GuiInventory.simulateMouseClickByBottom(1160, 830, true)); // 精炼物品
            
            Refine1.addAction(new DelayAction(12));
            Refine1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(730, 440, true)); // 第一格
            Refine1.addAction(new DelayAction(6));
            Refine1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(730, 440, true)); // 第一格
            
            Refine1.addAction(new DelayAction(6));
            Refine1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(1100, 820, false)); // 开始精炼
            Refine1.addAction(new DelayAction(6));
            Refine1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(1135, 475, true)); // 取出物品
            ...
            Refine1.addAction(new DelayAction(10));
            Refine1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(955, 595, true)); // 开始分解
            Refine1.addAction(new DelayAction(6));
            Refine1.addAction(player -> GuiInventory.simulateMouseClickByMiddle(955, 595, true)); // 开始分解
        ...
        // 模拟点击指定屏幕坐标处 xxx.addAction(player -> GuiInventory.simulateMouseClickByMiddle(1222, 356, true));
        ...
        public static void clickRefineItemsInInventory() {
            ...
            int[][] slotCoordinates = {
                //  9~44 每个槽位对应的 (x, y) 坐标
            		...
            		{670,780}, {745,780}, {815,780}, {890,780}, {960,780}, {1030,780}, {1100,780}, {1175,780}, {1250,780}, 
            		{670,855}, {745,855}, {815,855}, {890,855}, {960,855}, {1030,855}, {1100,855}, {1175,855}, {1250,855}, 
            		{670,945}, {745,945}, {815,945}, {890,945}, {960,945}, {1030,945}, {1100,945}, {1175,945}, {1250,945}
            };
            ...
            for (int i = 9; i <= 44; i++) { // 只处理 9~44 槽位
                ...
                if (containsWhitelist && !containsBlacklist) {
                    try {
                        ...
                        int x = coords[0];
                        int y = coords[1];
                        simulateMouseClickByMiddle(x, y, true);
                        new DelayAction(3, () -> {
                        	simulateMouseClickByMiddle(x + 10, y, true);
                        }).accept(player);
     ...
     // 修改加载显示设置方法
        private static void loadDisplaySettings() {
            try {
                ...
                if (Files.exists(configPath)) {
                    ...
                    // 添加默认值处理
                    customScreenWidth = obj.has("width") ? obj.get("width").getAsInt() : 1920;
                    customScreenHeight = obj.has("height") ? obj.get("height").getAsInt() : 1200;
                    customScreenScale = obj.has("scale") ? obj.get("scale").getAsInt() : 125;
                    customGuiScale = obj.has("guiscale") ? obj.get("guiscale").getAsDouble() : 4.0;
                    LOGGER.info("已加载自定义分辨率: {}x{} ({}% {}x)", customScreenWidth, customScreenHeight, customScreenScale, customGuiScale);
                }
            } catch (Exception e) {
        ...
        private static void saveDisplaySettings() {
            try {
                ...
                json.addProperty("guiscale", customGuiScale);
        ...
        public static class ResolutionInputGui extends GuiScreen {
            ...
            @Override
            public void initGui() {
                this.buttonList.add(new GuiButton(0, width/2 - 100, height/2 + 95, 200, 20, "保存配置"));
                
                widthField = new GuiTextField(1, this.fontRenderer, 
                    width/2 - 100, height/2 - 105, 200, 20);
                ...
                heightField = new GuiTextField(2, this.fontRenderer,
                    width/2 - 100, height/2 - 55, 200, 20);
                ...
                scaleField.setMaxStringLength(3);
                ...
                guiscaleField = new GuiTextField(4, this.fontRenderer,
                        width/2 - 100, height/2 + 45, 200, 20);
                guiscaleField.setMaxStringLength(2);
                guiscaleField.setText(customGuiScale > 0 ? String.valueOf(customGuiScale) : "");
            }
            ...
            protected void actionPerformed(GuiButton button) {
                if (button.id == 0) { // 保存按钮
                    try {
                        ...
                        customGuiScale = Double.parseDouble(guiscaleField.getText());
                        saveDisplaySettings();
            ...
            public void drawScreen(int mouseX, int mouseY, float partialTicks) {
                ...
                drawCenteredString(fontRenderer, "请输入屏幕分辨率", width/2, height/2 - 140, 0x632B30); // 红酒色
                drawString(fontRenderer, "宽度:", width/2 - 100, height/2 - 120, 0xE6AF2E); // 黄澄色
                drawString(fontRenderer, "高度:", width/2 - 100, height/2 - 70, 0xE6AF2E);
                drawString(fontRenderer, "缩放 %:", width/2 - 100, height/2 - 20, 0xE6AF2E);
                drawString(fontRenderer, "界面尺寸 x:", width/2 - 100, height/2 + 30, 0xE6AF2E);
                widthField.drawTextBox();
                heightField.drawTextBox();
                scaleField.drawTextBox();
                guiscaleField.drawTextBox();
            ...
            protected void keyTyped(char typedChar, int keyCode) throws IOException {
                ...
                guiscaleField.textboxKeyTyped(typedChar, keyCode);
            }
            ...
            protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
                ...
                guiscaleField.mouseClicked(mouseX, mouseY, mouseButton);
            }
        }
        ...
        /**
         * 模拟鼠标点击-基于中间点
         * @param x 鼠标X坐标（基于1920）
         * @param y 鼠标Y坐标（基于1200）
         * @param isLeftClick 是否为左键点击
         */
        // 一般的GUI
        public static void simulateMouseClickByMiddle(int x, int y, boolean isLeftClick) {
            Minecraft.getMinecraft().addScheduledTask(() -> {
                try {
                    // 中间点坐标
                    double middleX = customScreenWidth / 2;
                    double middleY = customScreenHeight / 2;
                    
                    // 计算分辨率缩放比例
                    double scale = customScreenScale / 100.0;
                    double guiscale = customGuiScale / 4.0;
                    
                    // 中点坐标偏移
                    double Xoffset = (x - 960) * guiscale;
                    double Yoffset = (y - 600) * guiscale;
                    
                    // 获取实际点击位置
                    int actualX = (int)((middleX + Xoffset) / scale);
                    int actualY = (int)((middleY + Yoffset) / scale);
                    ...
                    LOGGER.info("自动缩放点击 | 逻辑坐标: ({}, {}) 实际坐标: ({}, {}) 系统缩放: {} {}x", x, y, actualX, actualY, scale, guiscale);
                } catch (Exception e) {
         ...
         */
        // NPC对话
        public static void simulateMouseClickByBottom(int x, int y, boolean isLeftClick) {
            Minecraft.getMinecraft().addScheduledTask(() -> {
                try {
                	// 中间点坐标
                    double bottomX = customScreenWidth / 2;
                    double bottomY = customScreenHeight;
                    ...
                    // 底中点坐标偏移
                    double Xoffset = (x - 960) * guiscale;
                    double Yoffset = (y - 1200) * guiscale;
                    
                    // 获取实际点击位置
                    int actualX = (int)((bottomX + Xoffset) / scale);
                    int actualY = (int)((bottomY + Yoffset) / scale);
        ...
        private static void checkAutoEat(EntityPlayerSP player) {   
            if (!autoEatEnabled || isGuiOpen() || isEating || player.getFoodStats().getFoodLevel() > 12) return;
        ...
        public void drawScreen(int mouseX, int mouseY, float partialTicks) {
            ...
            if (currentCategory.equals("自动操作")) {
                ...
                // 新增自动技能状态显示
                String autoFollowStatus = "自动追怪 " + (autoSkillEnabled ? "✔" : "✘");
                fontRenderer.drawStringWithShadow(autoFollowStatus, x + 55, y + 190, 0xAFEEEE); // 灰绿松石色
                
                // 显示当前分辨率
                String resolutionStatus = "当前分辨率: " + (customScreenWidth > 0 ? customScreenWidth + "x" 
                                                        + customScreenHeight + " (" + customScreenScale + "% " 
                                                        + customGuiScale + "x)": "未设置");
                ...
                if (mouseX >= itemX && mouseX <= itemX + 30 && 
                        ...
                        if (command.startsWith("path:")) {
                        	...
                        	if (command.equals("autoeat")) {
                        		...
                        		mc.displayGuiScreen(new AutoFollowConfigGui());
                        		return;
                        	}
                        } else {
        ...
        public static class LoopCountInputGui extends GuiScreen {
            ...
            protected void actionPerformed(GuiButton button) throws IOException {
                ...
                if (button.id == 101) { // 自动进食开关
                    ...
                    button.displayString = "自动进食: " + (autoEatEnabled ? "✔" : "✘");
                }
            }
            ...
            // 自动追怪设置GUI
            public static class AutoFollowConfigGui extends GuiScreen {
                ...
                public void initGui() {
                    ...
                    point1XField = new GuiTextField(0, this.fontRenderer, inputX  , inputY + 10, 80, 20);
                    point1ZField = new GuiTextField(1, this.fontRenderer, inputX + 100, inputY + 10, 80, 20);
                    point2XField = new GuiTextField(2, this.fontRenderer, inputX , inputY + 50, 80, 20);
                    point2ZField = new GuiTextField(3, this.fontRenderer, inputX + 100, inputY + 50, 80, 20);

                    // 设置输入框验证器
                    point1XField.setValidator(this::validateCoordinate);
                    point1ZField.setValidator(this::validateCoordinate);
                    point2XField.setValidator(this::validateCoordinate);
                    point2ZField.setValidator(this::validateCoordinate);

                    // 设置输入框初始值
                    point1XField.setText(String.format("%.1f", point1.x));
                    point1ZField.setText(String.format("%.1f", point1.z));
                    point2XField.setText(String.format("%.1f", point2.x));
                    point2ZField.setText(String.format("%.1f", point2.z));

                    // 添加按钮
                    this.buttonList.add(new GuiButton(0, inputX, inputY + 90, 80, 20, "获取点1"));
                    this.buttonList.add(new GuiButton(1, inputX + 100, inputY + 90, 80, 20, "获取点2"));
                    this.buttonList.add(new GuiButton(2, inputX, inputY + 120, 180, 20, "保存配置"));
                    this.buttonList.add(new GuiButton(4, inputX, inputY + 150, 180, 20, 
                        "自动追怪: " + (Followenabled ? "✔" : "✘")));
                }
                ...
                public void drawScreen(int mouseX, int mouseY, float partialTicks) {
                    ...
                    // 绘制标题
                    drawCenteredString(fontRenderer, "自动追怪设置", width / 2 - 8, 30, 0xF29559); // 原橘色
                    
                    // 绘制标签
                    int labelX = width / 2 - 100;
                    drawString(fontRenderer, "点1坐标 (X, Z):", labelX, height / 2 - 60, 0xF2D492); // 桃黄色
                    drawString(fontRenderer, "点2坐标 (X, Z):", labelX, height / 2 - 20, 0xF2D492);
                    
                    // 绘制输入框
                    point1XField.drawTextBox();
                    point1ZField.drawTextBox();
                    point2XField.drawTextBox();
                    point2ZField.drawTextBox();
                    
                    // 绘制提示信息
                    if (!statusMessage.isEmpty()) {
                        drawCenteredString(fontRenderer, statusMessage, width/2 - 8, height/2 + 110, 0xFF0000);
                    }
                ...
                protected void actionPerformed(GuiButton button) {
                    try {
                        if (button.id == 0) { // 获取点1
                            ...
                            saveFollowConfig();
                        } else if (button.id == 1) { // 获取点2
                            updatePointFromPlayer(point2XField, point2ZField);
                            saveFollowConfig();
                        } else if (button.id == 2) { // 保存配置
                            saveFollowConfig();
                            ...
                            new Timer().schedule(new TimerTask() {
                                public void run() {
                                    Minecraft.getMinecraft().addScheduledTask(() -> {
                                        ...
                                        mc.displayGuiScreen(null);
                                        if (Followenabled) {
                                        	AutoFollowHandler.sendCommand(".stop");
                                        	AutoFollowHandler.sendCommand(".follow entities");
                                        }
                                    });
                                }
                            }, 200);
                            
                        } else if (button.id == 4) { // 开关切换
                            Followenabled = !Followenabled;
                            button.displayString = "自动追怪: " + (Followenabled ? "✔" : "✘");
                        }
                    } catch (Exception e) {
                    	statusMessage = "保存失败：" + e.getMessage();
                        LOGGER.error("自动追怪配置操作失败", e);
                    }
                }
                ...
                protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
                    super.mouseClicked(mouseX, mouseY, mouseButton);
                    point1XField.mouseClicked(mouseX, mouseY, mouseButton);
                    point1ZField.mouseClicked(mouseX, mouseY, mouseButton);
                    point2XField.mouseClicked(mouseX, mouseY, mouseButton);
                    point2ZField.mouseClicked(mouseX, mouseY, mouseButton);
                }
                ...
                public void updateScreen() {
                    ...
                    formatCoordinateInput(point1XField);
                    formatCoordinateInput(point1ZField);
                    formatCoordinateInput(point2XField);
                    formatCoordinateInput(point2ZField);
                }
                
                // 格式化方法
                private void formatCoordinateInput(GuiTextField field) {
                    ...
                    // 限制长度
                    if (input.length() > 12) {
                        field.setText(input.substring(0, 12));
                    }
                    
                    // 保留一位小数
                    if (input.contains(".") && input.split("\\.")[1].length() > 1) {
                        int dotIndex = input.indexOf(".");
                        field.setText(input.substring(0, dotIndex + 2)); // 保留一位小数
                    }
                }
                ...
                private void updatePointFromPlayer(GuiTextField xField, GuiTextField zField) {
                    Minecraft mc = Minecraft.getMinecraft();
                    if (mc.player != null) {
                        xField.setText(String.format("%.1f", mc.player.posX));
                        zField.setText(String.format("%.1f", mc.player.posZ));
                    }
                }
                ...
                private boolean validateCoordinate(String input) {
                    ...
                    try {
                        Double.parseDouble(input);
                        return input.length() <= 12; // 限制最大长度为12
                    } catch (NumberFormatException e) {
                        return false;
                    }
                }
                ...
                // 保存配置
                private void saveFollowConfig() {
                    try {
                        Point newPoint1 = new Point(
                            Double.parseDouble(point1XField.getText()),
                            Double.parseDouble(point1ZField.getText()));
                        Point newPoint2 = new Point(
                            Double.parseDouble(point2XField.getText()),
                            Double.parseDouble(point2ZField.getText()));
                        
                        // 创建JSON对象
                        JsonObject json = new JsonObject();
                        json.addProperty("enabled", Followenabled);
                        json.addProperty("point1X", newPoint1.x);
                        json.addProperty("point1Z", newPoint1.z);
                        json.addProperty("point2X", newPoint2.x);
                        json.addProperty("point2Z", newPoint2.z);
                        json.addProperty("minX", Math.min(newPoint1.x, newPoint2.x));
                        json.addProperty("maxX", Math.max(newPoint1.x, newPoint2.x));
                        json.addProperty("minZ", Math.min(newPoint1.z, newPoint2.z));
                        json.addProperty("maxZ", Math.max(newPoint1.z, newPoint2.z));
                        
                        // 创建配置文件
                        if (!FOLLOW_CONFIG_FILE.exists()) {
                            FOLLOW_CONFIG_FILE.getParentFile().mkdirs();
                            FOLLOW_CONFIG_FILE.createNewFile();
                        }
                        
                        // 写入文件
                        try (FileWriter writer = new FileWriter(FOLLOW_CONFIG_FILE)) {
                            writer.write(json.toString());
                        }
                        ...
                        statusMessage = "保存失败: 无效输入";
                        LOGGER.error("保存自动追怪配置失败", e);
                    }
                }
            }

            // 加载配置
            public static void loadFollowConfig() {
                try {
                    if (FOLLOW_CONFIG_FILE.exists()) {
                        String jsonStr = new String(Files.readAllBytes(Paths.get(FOLLOW_CONFIG_FILE.toURI())));
                        JsonObject json = new JsonParser().parse(jsonStr).getAsJsonObject();
                        
                        Followenabled = json.has("enabled") && json.get("enabled").getAsBoolean();
                        
                        if (json.has("point1X") && json.has("point1Z")) {
                            point1 = new Point(
                                json.get("point1X").getAsDouble(),
                                json.get("point1Z").getAsDouble());
                        }
                        
                        if (json.has("point2X") && json.has("point2Z")) {
                            point2 = new Point(
                                json.get("point2X").getAsDouble(),
                                json.get("point2Z").getAsDouble());
                        }
                        
                        if (json.has("point1X") && json.has("point2X")) {
                            minX = Math.min(json.get("point1X").getAsDouble(), json.get("point2X").getAsDouble());
                            maxX = Math.max(json.get("point1X").getAsDouble(), json.get("point2X").getAsDouble());
                        }
                        
                        if (json.has("point1Z") && json.has("point2Z")) {
                        	minZ = Math.min(json.get("point1Z").getAsDouble(), json.get("point2Z").getAsDouble());
                        	maxZ = Math.max(json.get("point1Z").getAsDouble(), json.get("point2Z").getAsDouble());
                        }
                    ...
                    LOGGER.error("加载自动追怪配置失败", e);
                }
            }

        // 自动追怪核心逻辑  
        public static class AutoFollowHandler {
        	
            // 添加事件处理方法
            @SubscribeEvent
            public void onPlayerTick(TickEvent.PlayerTickEvent event) {
            	if (event.player == null || !event.player.equals(Minecraft.getMinecraft().player)) return;
                if (!Followenabled || point1 == null || point2 == null) return;
                ...
                // 检测是否超出范围
                if (!isMovingToPoint && !isWithinBounds) {
                	isMovingToPoint = true;
                    sendCommand(".stop");
                    sendCommand(".goto " + point1.x + " " + point1.z);
                }
                
                // 检测是否到达中心点
                if (isMovingToPoint && center != null && isWithinBounds) {
                	double dx = playerX - point1.x;
                    double dz = playerZ - point1.z;
                    double distanceSquared = Math.pow(dx, 2) + Math.pow(dz, 2);
                    
                    if (distanceSquared <= Math.pow(DETECTION_RADIUS, 2)) {
                    	
                    	isMovingToPoint = false;
                    	sendCommand(".stop");
                        sendCommand(".follow entities");
                        center = null;
                    }
                }
            }
